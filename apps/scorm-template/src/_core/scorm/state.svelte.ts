...
class StudentState {
  ...
  load(wrapper: ScormWrapper): void {
    ...
    this.#id = wrapper.getValue(
      isV2004 ? SCORM2004.LEARNER_ID : SCORM12.CORE_STUDENT_ID,
    );
    this.#name = wrapper.getValue(
      isV2004 ? SCORM2004.LEARNER_NAME : SCORM12.CORE_STUDENT_NAME,
    );
  }
}
...
class SessionState {
  ...
  load(wrapper: ScormWrapper): void {
    ...
    this.#entry = wrapper.getValue(
      isV2004 ? SCORM2004.ENTRY : SCORM12.CORE_ENTRY,
    ) as Scorm2004Entry | "ab-initio";
  }
}
...
export class ScormState {
  ...
  set location(value: string) {
    this.#location = value;
    const element =
      this.#wrapper.version === "2004"
        ? SCORM2004.LOCATION
        : SCORM12.CORE_LESSON_LOCATION;
  ...
  initialize({
    ...
    const engine =
      storageMode === "chunked"
        ? new ChunkedStorageEngine()
        : new StandardStorageEngine();
  ...
  terminate(): void {
    ...
    const timeElement =
      this.#wrapper.version === "2004"
        ? SCORM2004.SESSION_TIME
        : SCORM12.CORE_SESSION_TIME;
